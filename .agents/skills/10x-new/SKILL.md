...
1. **Sprawdzenie kebab-case**: `<change-id>` musi pasować do `^[a-z][a-z0-9]*(-[a-z0-9]+)*$` (zaczyna się od litery, segmenty małych liter + cyfry oddzielone pojedynczymi myślnikami, bez początkowego/końcowego myślnika, bez podwójnych myślników).
...
4. Zapisz `context/changes/<change-id>/change.md` w dokładnie takim kształcie (slot `<notes-body>` to to, co zostało wyprodukowane w kroku 3):
...
- Nie zapisuje `frame.md`, `research.md`, `plan.md` ani żadnych innych artefaktów — te pochodzą z odpowiednich umiejętności.
